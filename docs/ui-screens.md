...
| **Overview** | 선택 cycle의 수입, 현금유출, 소비분석 합계와 카테고리별 지출 및 계좌별 추정잔액 요약. 상단에서 cycle 범위와 주간 복사 대상 기간 표시 |
| **Transactions** | 거래 목록, 검색/흐름/계좌/카테고리/플래그/기간 필터, 날짜 고정 + 시:분 후보정, Account 컬럼, Cash/Spend/소액결제/Adjusted 플래그 표시 |
