...
| **Overview** | 선택 cycle과 optional week 기준의 수입, 이체 제외 External Cash Out, Deferred Spending, Actual Spending, Net External Cashflow, 전체 카테고리별 지출과 원형 Category Mix, 은행/현금/목적자금 Account Flow(`Opening / In / Out / Net / Closing`), 소액결제/후불 Liability Flow 요약. Flow 영역은 긴 금액/계좌명을 위해 별도 row와 고정 컬럼으로 표시. Opening은 seed + 이전 구간 net, Closing = Opening + In − Out |
...
| **재무 요약 복사** | "Copy Cycle Summary" 또는 "Copy Weekly Summary" 버튼 → 선택 cycle 또는 선택 week의 재무 요약을 마크다운으로 클립보드 복사. Totals, 전체 Spending Categories, Account Flow(Opening~Closing), Liability Flow를 포함하며, week는 월요일~일요일 기준이되 cycle 시작/끝 주는 cycle 범위로 잘라 표시 |
| **Accounts** | 선택 cycle/week 기준 계좌 흐름(Seed/Opening/In/Out/Net/Closing), 계좌/지갑/부채/목적자금 카드, alias 관리, seed opening balance 보정, unmapped asset 생성/기존 계좌 매핑 |
