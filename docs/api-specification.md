...
거래 응답은 원본 금액 `amount`와 분석용 금액 `cashflowAmount`, `spendingAmount`를 함께 내려준다. Spending 합계와 카테고리 합계는 `spendingAmount`를 사용하며, 통신비 청구 row는 같은 cycle의 `소액결제` 합계를 제외한 나머지만 소비분석 금액으로 잡는다. External Out은 `이체지출`을 제외한 외부 현금유출만 의미하고, Account Flow는 계좌 간 이체 입출금을 포함한 실제 계좌별 흐름을 의미한다.
