...
| **계좌/부채성 흐름** | Account Flow는 Opening/In/Out/Net/Closing으로 표시(이체 입출금 포함), Liability Flow는 소액결제/후불 미정산 흐름, External Cash Out은 이체 제외 외부 현금유출로 구분 |
| **잔액 보정** | 계좌별 seed opening balance를 저장하고, 선택 salary cycle Opening = seed + 이전 cycle net, Closing = Opening + 구간 net으로 계산 |
